...
contract Kenome is KIP17Token, Ownable {
    uint256 private _idCounter;
    string private _baseTokenURI;

    constructor() KIP17Token("KlayGenome", "KENOME") public {
    }
    ...
    function safeBulkTransfer(address[] calldata tos, uint256[] calldata ids) external {
        uint256 length = ids.length;
        for (uint i = 0; i < length; ++i) {
